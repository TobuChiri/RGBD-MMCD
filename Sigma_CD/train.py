...
with Engine(custom_parser=parser) as engine:
    ...
    for epoch in range(engine.state.epoch, config.nepochs+1):
        ...
        #TODO:固定encoder/decoder
        if epoch == config.change_epochs + 1:
            if engine.distributed:
                for param in model.module.backbone.parameters():
                    param.requires_grad = False
                    param.detach()
                model.module.backbone.eval()
                for param in model.module.decode_head.parameters():
                    param.requires_grad = False
                    param.detach()
                model.module.decode_head.eval()
                params = model.module.parameters()
                optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad,params), lr=lr, betas=(0.9, 0.999), weight_decay=config.weight_decay)
            else:
                ...
                optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad,params), lr=lr, betas=(0.9, 0.999), weight_decay=config.weight_decay)
                # stage2_params_list = []
                # for item in filter(lambda p: p.requires_grad, params):
                #     stage2_params_list.append(item)
                # optimizer = torch.optim.AdamW(stage2_params_list, lr=lr)
        ...
        for idx in pbar:
            ...
            aux_rate = 0.2
            loss = model(imgs, modal_xs, epoch, config.change_epochs, gts)
        ...
        if engine.distributed:
                ...
                if (epoch >= config.checkpoint_start_epoch) and (epoch - config.checkpoint_start_epoch) % config.checkpoint_step == 0:
                    ...
                    with torch.no_grad():
                        ...
                        _, mean_IoU = segmentor.run(config.checkpoint_dir, str(epoch), config.val_log_file,
                                    config.link_val_log_file, epoch)
            ...
            if (epoch >= config.checkpoint_start_epoch) and (epoch - config.checkpoint_start_epoch) % config.checkpoint_step == 0:
                ...
                model.train()
        #TODO:复制一份encoder
        if epoch == config.change_epochs:
            if engine.distributed:
                model.module.backbone_ctrl.load_state_dict(model.module.backbone.state_dict())
            else:
