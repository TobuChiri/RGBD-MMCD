...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device, config, epoch):
        img = data['data']
        # img = cv2.resize(img, (640, 480))
        label = data['label']
        modal_x = data['modal_x']
        modal_x = cv2.resize(modal_x, (640, 480))
        name = data['fn']
        pred = self.sliding_eval_rgbX(img, modal_x, config.eval_crop_size, config.eval_stride_rate, epoch, device)
