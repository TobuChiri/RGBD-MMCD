...
from configs.config_nyu import config#TODO
...
class Evaluator(object):
    ...
    def run(self, model_path, model_indice, log_file, log_file_link, epoch):
        ...
        for model in models:
            ...
            if len(self.devices ) == 1:
                result_line, mean_IoU = self.single_process_evalutation(epoch)
            else:
    ...
    def run_eval(self, model_path, model_indice, log_file, log_file_link, epoch=None):
        """There are four evaluation modes:
            1.only eval a .pth model: -e *.pth
            2.only eval a certain epoch: -e epoch
            3.eval all epochs in a given section: -e start_epoch-end_epoch
    ...
    def single_process_evalutation(self, epoch):
        ...
        for idx in tqdm(range(self.ndata)):
            dd = self.dataset[idx]
            results_dict = self.func_per_iteration(dd,self.devices[0], self.config, epoch)
    ...
    def sliding_eval_rgbX(self, img, modal_x, crop_size, stride_rate, epoch, device=None):
        ...
        for s in self.multi_scales:
            ...
            processed_pred += self.scale_process_rgbX(img_scale, modal_x_scale, (ori_rows, ori_cols),
                                                        crop_size, stride_rate, epoch, device)

        pred = processed_pred.argmax(2)

        return pred

    def scale_process_rgbX(self, img, modal_x, ori_shape, crop_size, stride_rate, epoch, device=None):
        new_rows, new_cols, c = img.shape
        long_size = new_cols if new_cols > new_rows else new_rows
        ...
        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            ...
            score = self.val_func_process_rgbX(input_data, input_modal_x, epoch, device) 
    ...
    def val_func_process_rgbX(self, input_data, input_modal_x, epoch, device=None):
        ...
        change_epoch = config.change_epochs
        with torch.cuda.device(input_data.get_device()):
            self.val_func.eval()
            self.val_func.to(input_data.get_device())
            with torch.no_grad():
                score = self.val_func(input_data, input_modal_x, epoch, change_epoch)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
                    input_modal_x = input_modal_x.flip(-1)
                    score_flip = self.val_func(input_data, input_modal_x, epoch, change_epoch)
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                score = torch.exp(score)
