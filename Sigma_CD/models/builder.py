...
class zeroConv(nn.Module):
    def __init__(self):
        ...
        self.last1_ctrl_zero = zero_module(nn.Conv2d(96, 96, kernel_size=1))
        self.last2_ctrl_zero = zero_module(nn.Conv2d(192, 192, kernel_size=1))
        self.last3_ctrl_zero = zero_module(nn.Conv2d(384, 384, kernel_size=1))
        self.last4_ctrl_zero = zero_module(nn.Conv2d(768, 768, kernel_size=1))

    def forward(self, last_x_feature_ctrl):
        last_x_feature_ctrl[0] = self.last1_ctrl_zero(last_x_feature_ctrl[0])
        last_x_feature_ctrl[1] = self.last2_ctrl_zero(last_x_feature_ctrl[1])
        last_x_feature_ctrl[2] = self.last3_ctrl_zero(last_x_feature_ctrl[2])
        last_x_feature_ctrl[3] = self.last4_ctrl_zero(last_x_feature_ctrl[3])
        return last_x_feature_ctrl
    
def modality_drop(train_mode, x_rgb, x_depth):
    if train_mode:
...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        elif cfg.backbone == 'sigma_small':#TODO
            logger.info('Using backbone: V-MAMBA')
            self.channels = [96, 192, 384, 768]
            from .encoders.dual_vmamba import vssm_small as backbone
            self.backbone = backbone()
            self.backbone_ctrl = backbone()
    ...
    def encode_decode(self, rgb, modal_x, epoch, change_epoch):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        if not self.deep_supervision:#TODO
            orisize = rgb.shape
            if self.training:
                if epoch > change_epoch:
                    rgb, modal_x, p = modality_drop(self.training, rgb, modal_x)
            rgb_copy = rgb
            modal_x_copy = modal_x
            x = self.backbone(rgb, modal_x)
            if self.training:
                if epoch > change_epoch:
                    x_ctrl = self.backbone_ctrl(rgb_copy, modal_x_copy)
    ...
    def forward(self, rgb, modal_x, epoch, change_epoch, label=None):
        if not self.deep_supervision:
            if self.aux_head:
                out, aux_fm = self.encode_decode(rgb, modal_x)
            else:
                out = self.encode_decode(rgb, modal_x, epoch, change_epoch)
