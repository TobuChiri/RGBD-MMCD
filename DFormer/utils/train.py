...
with Engine(custom_parser=parser) as engine:
    ...
    if dataset_name == "NYUDepthv2":
        weight_path = 'checkpoints/NYUv2_DFormer_Base.pth'
        print(f'Loading NYU Depth v2 pretrained weights from {weight_path}')
    elif dataset_name == "SUNRGBD":
        weight_path = 'checkpoints/SUNRGBD_DFormer_Base.pth'
        print(f'Loading SUNRGBD pretrained weights from {weight_path}')
    else:
        print(f'Warning: No specific pretrained weights for dataset {dataset_name}')
        print('Using SUNRGBD weights as default')
        weight_path = 'checkpoints/SUNRGBD_DFormer_Base.pth'
    ...
    model.backbone_control.load_state_dict(model.backbone.state_dict())
    ...
    for epoch in range(engine.state.epoch, config.nepochs + 1):
        ...
        for idx in range(config.niters_per_epoch):
            ...
            if isinstance(loss, torch.Tensor):
                logger.info(f"Loss shape: {loss.shape}, numel: {loss.numel()}, dim: {loss.dim()}")
                if loss.dim() > 0:
                    logger.info(f"Loss is not scalar! Trying to convert to scalar...")
                    loss = loss.mean() if loss.numel() > 0 else torch.tensor(0.0, device=loss.device)
                    logger.info(f"After mean: shape: {loss.shape}, numel: {loss.numel()}")
